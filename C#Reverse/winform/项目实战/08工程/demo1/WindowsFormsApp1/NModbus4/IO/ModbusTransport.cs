namespace Modbus.IO {
    ...
    public abstract class ModbusTransport : IDisposable {
        ...
        //写入完成 - 事件处理
        public event EventHandler<ModbusTransPortEventArgs> ModbusTransPortWriteCompleted;
        ...
        public int WaitToRetryMilliseconds {
            ...
            set {
                if (value < 0)
                    throw new ArgumentException(ModbusResource.WaitRetryGreaterThanZero);
            ...
            do {
                try {
                    lock (_syncLock) {
                        ...
                        //通知数据写入成功
                        var eventArgs = new ModbusTransPortEventArgs(BuildMessageFrame(message));
                        ModbusTransPortWriteCompleted.Raise(this, eventArgs);
