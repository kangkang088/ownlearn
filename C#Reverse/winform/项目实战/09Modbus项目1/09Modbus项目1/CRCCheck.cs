...
namespace _09Modbus项目1 {
    public static class CRCCheck {
        ...
        /// <returns></returns>
        public static byte[] CalculateCRC16(byte[] pucFrame, int usLen) {
        ...
        /// <returns></returns>
        public static bool CRCVerify(byte[] res) {
            if(res == null) return false;
            if(res.Length <= 2) return false;

            //截取需要校验的
            byte[] data = new byte[res.Length - 2];
            Array.Copy(res, 0, data, 0, res.Length - 2);
            byte[] crc = CalculateCRC16(data,data.Length);
            return crc[0] == res[res.Length - 2] && crc[1] == res[res.Length - 1];
        }
    }
}
